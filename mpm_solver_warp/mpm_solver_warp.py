...
class MPM_Simulator_WARP:
    ...
    def set_parameters_dict(self, kwargs={}, device="cuda:0"):
        ...
        if "E" in kwargs:
            if isinstance(kwargs["E"], dict):
                E_array = np.full(self.n_particles, 0, dtype=np.float32)
                current_particle_index = 0
                for object in kwargs["objects"]:
                    E_array[current_particle_index:current_particle_index+kwargs["objpart"][object]] = float(kwargs["E"][object])
                    current_particle_index += kwargs["objpart"][object]
                E_wrap = wp.from_numpy(E_array, dtype=float, device=device)
                self.mpm_model.E = E_wrap
            else:
                wp.launch(
        ...
        if "density" in kwargs:
            if isinstance(kwargs["density"], dict):
                density_array = np.full(self.n_particles, 0, dtype=np.float32)
                current_particle_index = 0
                for obj in kwargs["objects"]:
                    num = kwargs["objpart"][obj]
                    density_array[current_particle_index:current_particle_index+num] = float(kwargs["density"][obj])
                    current_particle_index += num
                density_wrap = wp.from_numpy(density_array, dtype=float, device=device)
                self.mpm_state.particle_density = density_wrap
            else:
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time:
                if grid_x < padding and state.grid_v_out[grid_x, grid_y, grid_z][0] < 0:
                    state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(
                        0.0,
                        state.grid_v_out[grid_x, grid_y, grid_z][1] * friction,
                        state.grid_v_out[grid_x, grid_y, grid_z][2] * friction,
                    )
                if (
                ...
                if grid_y < padding and state.grid_v_out[grid_x, grid_y, grid_z][1] < 0:
                    state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(
                        state.grid_v_out[grid_x, grid_y, grid_z][0] * friction,
                        0.0,
                        state.grid_v_out[grid_x, grid_y, grid_z][2] * friction,
                    )
                if (
