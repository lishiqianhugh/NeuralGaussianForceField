...
def load_images_as_tensor(path='data/truck', interval=1, PIXEL_LIMIT=255000):
    ...
    if osp.isdir(path):
        ...
        filenames = sorted([x for x in os.listdir(path) if x.lower().endswith(('.png', '.jpg', '.jpeg')) and not (x.lower().endswith('_depth.png'))])
    ...
    if PIXEL_LIMIT is None:
        k = max(1, W_orig // 14)
        m = max(1, H_orig // 14)
        TARGET_W, TARGET_H = k * 14, m * 14
        if TARGET_W >= W_orig and TARGET_H >= H_orig:
            TARGET_W, TARGET_H = W_orig, H_orig
        else:
            TARGET_W = min(TARGET_W, W_orig)
            TARGET_H = min(TARGET_H, H_orig)
    else:
...
def resize_images(images, PIXEL_LIMIT=201000):
    ...
    is_int = not torch.is_floating_point(images)
    ...
    if PIXEL_LIMIT is None:
        ...
        if tw >= W and th >= H:
            ...
            tw = min(tw, W)
            th = min(th, H)
            mode = 'nearest' if is_int or C == 1 else 'bilinear'
            out = F.interpolate(images.float() if is_int else images, size=(int(th), int(tw)), mode=mode, align_corners=False if mode == 'bilinear' else None)
            if is_int:
                out = out.round().to(images.dtype)
