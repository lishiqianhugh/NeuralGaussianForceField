...
class GsplatRenderTabState(RenderTabState):
    ...
    # filtering controls
    top_conf_percent: float = 100.0  # 0-100, filter by depth_conf top percentage
    available_labels: Tuple[int, ...] = tuple()
    selected_labels: Tuple[int, ...] = tuple()
...
class GsplatViewer(Viewer):
    ...
    def _init_rendering_tab(self):
        ...
        self._gsplat_folder = None
        self._labels_folder = None

    def _populate_rendering_tab(self):
        server = self.server
        with self._rendering_folder:
            # Create and cache the Gsplat section folder so we can inject dynamic sub-sections later
            self._gsplat_folder = server.gui.add_folder("Gsplat")
            with self._gsplat_folder:
                ...
                top_conf_percent_slider = server.gui.add_number(
                    "Top Conf %",
                    initial_value=self.render_tab_state.top_conf_percent,
                    min=0.0,
                    max=100.0,
                    step=1.0,
                    hint="Render only the top X percent Gaussians by depth confidence (if available).",
                )

                @top_conf_percent_slider.on_update
                def _(_) -> None:
                    self.render_tab_state.top_conf_percent = float(top_conf_percent_slider.value)
                    self.rerender(_)
        ...
        self._rendering_tab_handles.update(
            {
                ...
                "top_conf_percent_slider": top_conf_percent_slider,
            }
        )
        super()._populate_rendering_tab()

    def set_available_labels(self, labels: Sequence[int]):
        """Create/refresh label filter checkboxes based on provided labels.
        Call this after the viewer is created and data is loaded.
        """
        server = self.server
        # De-duplicate and sort
        uniq = sorted(set(int(x) for x in labels))
        self.render_tab_state.available_labels = tuple(uniq)
        # Default: select all labels initially
        self.render_tab_state.selected_labels = tuple(uniq)

        # Create a Labels subfolder under the Gsplat section if not exists
        if self._labels_folder is None:
            # Ensure the Gsplat folder exists
            if self._gsplat_folder is None:
                self._gsplat_folder = server.gui.add_folder("Gsplat")
            with self._gsplat_folder:
                self._labels_folder = server.gui.add_folder("Labels")

        # Clear previous handles if any
        # Note: viser doesn't expose explicit removal; we can recreate checkboxes and overwrite handles
        label_checkboxes: Dict[int, Any] = {}
        with self._labels_folder:
            for lbl in uniq:
                name = f"BG(0)" if lbl == 0 else f"Label {lbl}"
                cb = server.gui.add_checkbox(
                    name,
                    initial_value=True,
                    hint="Toggle visibility of this label.",
                )

                @cb.on_update  # type: ignore
                def _(_evt=None, _lbl=lbl, _cb=cb) -> None:  # bind current values
                    # Recompute selected labels from current checkboxes
                    selected = []
                    for k, v in label_checkboxes.items():
                        if v.value:
                            selected.append(k)
                    # Ensure the one updated reflects its new value
                    if _cb.value and _lbl not in selected:
                        selected.append(_lbl)
                    elif (not _cb.value) and _lbl in selected:
                        selected.remove(_lbl)
                    self.render_tab_state.selected_labels = tuple(sorted(selected))
                    self.rerender(_evt)

                label_checkboxes[lbl] = cb

        self._rendering_tab_handles["label_checkboxes"] = label_checkboxes
