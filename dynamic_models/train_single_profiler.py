...
def parse_args():
    ...
    parser.add_argument('--profiler', action='store_true', help='Enable torch.profiler and write traces to TensorBoard')
    parser.add_argument('--profiler_logdir', type=str, default=None, help='Directory to write profiler traces (default: <save_dir>/profiler)')
...
if __name__ == "__main__":
    ...
    # Optional PyTorch profiler (writes traces consumable by TensorBoard)
    prof = None
    prof_logdir = None
    if args.profiler:
        prof_logdir = args.profiler_logdir or os.path.join(save_dir, 'profiler')
        os.makedirs(prof_logdir, exist_ok=True)
        tb_handler = tensorboard_trace_handler(prof_logdir)
        prof = profile(
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            # advance profiler state so traces are flushed per schedule
            if prof is not None:
                prof.step()
    ...
    # Stop profiler and flush any pending traces
    if prof is not None:
        try:
            prof.stop()
        except Exception as _:
            # do not fail training if profiler stop fails
            logging.warning("Profiler stop raised an exception")
